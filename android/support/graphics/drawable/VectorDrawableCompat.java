...
public class VectorDrawableCompat extends VectorDrawableCommon {
  ...
  private void printGroupTree(VGroup paramVGroup, int paramInt) {
    ...
    Log.v("VectorDrawableCompat", stringBuilder.toString());
    for (i = bool; i < paramVGroup.mChildren.size(); i++) {
      VObject vObject = paramVGroup.mChildren.get(i);
  ...
  private void updateStateFromTypedArray(TypedArray paramTypedArray, XmlPullParser paramXmlPullParser) {
    ...
    vPathRenderer.setAlpha(TypedArrayUtils.getNamedFloat(paramTypedArray, (XmlPullParser)stringBuilder, "alpha", 4, vPathRenderer.getAlpha()));
  ...
  private static class VGroup extends VObject {
    ...
    public boolean onStateChanged(int[] param1ArrayOfint) {
      ...
      while (i < this.mChildren.size()) {
        bool |= ((VectorDrawableCompat.VObject)this.mChildren.get(i)).onStateChanged(param1ArrayOfint);
        i++;
      } 
      return bool;
    }
  ...
  private static class VPathRenderer {
    ...
    private void drawGroupTree(VectorDrawableCompat.VGroup param1VGroup, Matrix param1Matrix, Canvas param1Canvas, int param1Int1, int param1Int2, ColorFilter param1ColorFilter) {
      ...
      param1Canvas.save();
      int i;
      for (i = 0; i < param1VGroup.mChildren.size(); i++) {
        VectorDrawableCompat.VObject vObject = param1VGroup.mChildren.get(i);
    ...
    private void drawPath(VectorDrawableCompat.VGroup param1VGroup, VectorDrawableCompat.VPath param1VPath, Canvas param1Canvas, int param1Int1, int param1Int2, ColorFilter param1ColorFilter) {
      ...
      if (((VectorDrawableCompat.VFullPath)param1VPath).mTrimPathStart != 0.0F || ((VectorDrawableCompat.VFullPath)param1VPath).mTrimPathEnd != 1.0F) {
        float f6 = ((VectorDrawableCompat.VFullPath)param1VPath).mTrimPathStart;
        float f7 = ((VectorDrawableCompat.VFullPath)param1VPath).mTrimPathOffset;
        float f4 = ((VectorDrawableCompat.VFullPath)param1VPath).mTrimPathEnd;
...
/* Location:              C:\Users\15856\Desktop\2675384-dex2jar.jar!\android\support\graphics\drawable\VectorDrawableCompat.class
