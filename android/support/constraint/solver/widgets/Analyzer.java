...
public class Analyzer {
  ...
  private static int getMaxDimension(ConstraintWidgetGroup paramConstraintWidgetGroup, int paramInt) {
    ...
    while (i < m) {
      ...
      int n = k + 1;
      if ((arrayOfConstraintAnchor[n]).mTarget == null || ((constraintWidget.mListAnchors[k]).mTarget != null && (constraintWidget.mListAnchors[n]).mTarget != null)) {
        bool = true;
      } else {
        bool = false;
      } 
      j = Math.max(j, getMaxDimensionTraversal(constraintWidget, paramInt, bool, 0));
      i++;
    } 
  ...
  public static void setPosition(List<ConstraintWidgetGroup> paramList, int paramInt1, int paramInt2) {
    ...
    for (int i = 0; i < j; i++) {
      for (ConstraintWidget constraintWidget : ((ConstraintWidgetGroup)paramList.get(i)).getWidgetsToSet(paramInt1)) {
...
/* Location:              C:\Users\15856\Desktop\2675384-dex2jar.jar!\android\support\constraint\solver\widgets\Analyzer.class
